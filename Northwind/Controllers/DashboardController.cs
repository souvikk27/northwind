...
namespace Northwind.Controllers
{
    public class DashboardController : Controller
    {
        ...
        public IActionResult Orders(int page = 1, string searchString = "")
        {
            ...
            {
                ordersQuery +=
                    " WHERE c.CompanyName LIKE @searchString OR "
                    + "e.FirstName + ' ' + e.LastName LIKE @searchString";
            }

            var ordersResult = connection.Query<OrderListVm>(
                ordersQuery,
        ...
        public IActionResult Customers(int page = 1, string searchString = "")
        {
            ...
            customersQuery +=
                " GROUP BY c.CustomerID, c.CompanyName, c.ContactName, c.ContactTitle, c.Country, c.Phone";

            var customersResult = connection.Query<CustomerListVm>(
                customersQuery,
                new { searchString = $"%{searchString}%" }
            );
        ...
        public IActionResult Suppliers(int page = 1, string searchString = "")
        {
            ...
            var suppliersQuery =
                "SELECT s.SupplierID, s.CompanyName, s.ContactName, s.ContactTitle, "
            ...
            {
                suppliersQuery +=
                    " WHERE s.CompanyName LIKE @searchString OR "
                    + "s.ContactName LIKE @searchString OR "
                    + "s.Country LIKE @searchString";
            }

            suppliersQuery +=
                " GROUP BY s.SupplierID, s.CompanyName, s.ContactName, s.ContactTitle, s.Country, s.Phone";

            var suppliersResult = connection.Query<SupplierListVm>(
                suppliersQuery,
                new { searchString = $"%{searchString}%" }
            );
        ...
        public IActionResult Employees(int page = 1, string searchString = "")
        {
            ...
            {
                employeesQuery +=
                    " WHERE e.FirstName LIKE @searchString OR "
                    + "e.LastName LIKE @searchString OR "
                    + "e.Title LIKE @searchString";
            }

            employeesQuery +=
                " GROUP BY e.EmployeeID, e.FirstName, e.LastName, e.Title, e.City, e.Country, e.HireDate, e.ReportsTo";

            var employeesResult = connection.Query<EmployeeListVm>(
                employeesQuery,
                new { searchString = $"%{searchString}%" }
            );
        ...
        public IActionResult Reports()
        {
            ...
            // Sales by Country
            var salesByCountryQuery =
                "SELECT c.Country, SUM(od.UnitPrice * od.Quantity * (1 - od.Discount)) as TotalSales "
                + "FROM [Order Details] od "
            ...
            var topProductsQuery =
                "SELECT TOP 5 p.ProductName, SUM(od.UnitPrice * od.Quantity * (1 - od.Discount)) as TotalSales "
                + "FROM [Order Details] od "
                + "JOIN Products p ON od.ProductID = p.ProductID "
        ...
        public IActionResult Earnings()
        {
            ...
            var viewModel = new EarningsViewModel
            {
                ...
                ProjectedAnnualEarnings =
                    monthlyEarnings.Sum(e => e.NetEarnings) * 12 / monthlyEarnings.Count,
            };
        ...
        public IActionResult Taxes()
        {
            ...
            var taxDocuments = new List<TaxDocumentVm>
            {
                new TaxDocumentVm
                {
                    DocumentName = "W-2 Form",
                    Year = now.Year - 1,
                    DateIssued = new DateTime(now.Year, 1, 31),
                    Status = "Available",
                    DownloadUrl = "/documents/taxes/w2-2022.pdf",
                },
                new TaxDocumentVm
                {
                    DocumentName = "1099-MISC",
                    Year = now.Year - 1,
                    DateIssued = new DateTime(now.Year, 1, 31),
                    Status = "Available",
                    DownloadUrl = "/documents/taxes/1099-misc-2022.pdf",
                },
                new TaxDocumentVm
                {
                    DocumentName = "Tax Return",
                    Year = now.Year - 1,
                    DateIssued = new DateTime(now.Year, 4, 15),
                    Status = "Filed",
                    DownloadUrl = "/documents/taxes/tax-return-2022.pdf",
                },
                new TaxDocumentVm
                {
                    DocumentName = "W-2 Form",
                    Year = now.Year - 2,
                    DateIssued = new DateTime(now.Year - 1, 1, 31),
                    Status = "Archived",
                    DownloadUrl = "/documents/taxes/w2-2021.pdf",
                },
                new TaxDocumentVm
                {
                    DocumentName = "Tax Return",
                    Year = now.Year - 2,
                    DateIssued = new DateTime(now.Year - 1, 4, 15),
                    Status = "Archived",
                    DownloadUrl = "/documents/taxes/tax-return-2021.pdf",
                },
            };
            ...
            var viewModel = new TaxesViewModel
            {
                ...
                EffectiveTaxRate =
                    monthlyTaxes.Sum(t => t.TotalTax) / monthlyTaxes.Sum(t => t.Income) * 100,
        ...
        public IActionResult Refunds()
        {
            ...
            for (int i = 0; i < 15; i++)
            {
                ...
                var processedDate =
                    status == "Pending" ? (DateTime?)null : createdDate.AddDays(random.Next(1, 7));
            ...
            var approvedRefunds = refundRequests.Count(r =>
            ...
            var refundsByReason = reasons
                .Select(reason => new RefundByReasonVm
                {
                    Reason = reason,
            ...
            var viewModel = new RefundsViewModel
            {
                ...
                RefundPolicy =
                    "Our standard refund policy allows customers to request refunds within 30 days of purchase. All refunds are subject to review and approval based on our terms and conditions.",
            };
